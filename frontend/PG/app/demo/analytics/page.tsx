...
import { MOCK_PRINTERS, MOCK_HISTORY, MOCK_ALERTS, MOCK_FLEET_METRICS } from "@/lib/mock-data";
...
export default function DemoAnalyticsPage() {
  ...
  const recentAlerts = MOCK_ALERTS;
  const jobs = MOCK_HISTORY;

  const failedJobs = jobs.filter((job) => job.status === "failed" || job.status === "warned").length;
  const completedJobs = jobs.filter((job) => job.status === "passed").length;
  ...
  return (
    <div className="animate-fade-in">
      <div className="px-6 pt-6 pb-4 border-b border-border">
        <h1 className="text-lg font-bold text-foreground tracking-tight">Analytics (Demo)</h1>
